package practice.graph.connectivity;

import org.junit.Test;
import practice.graph.ConnectivityGraph;

import java.util.Arrays;
import java.util.LinkedList;
import java.util.List;
import java.util.stream.Collectors;

import static org.hamcrest.CoreMatchers.equalTo;
import static org.junit.Assert.assertThat;

public class ArticulationPoint {

    /*
    We don't maintain 3 colors since
    undirected graph doesn't have forward and cross edge.
     */
    boolean[] visited;
    int[] parent;
    int[] disc;
    int[] low;

    List<Integer> results;
    int time = 0;
    ConnectivityGraph graph;

    public List<Integer> find(ConnectivityGraph graph) {
        time = 0;
        this.graph = graph;
        visited = new boolean[graph.V];

        parent = new int[graph.V];
        Arrays.fill(parent, -1);

        disc = new int[graph.V];
        low = new int[graph.V];
        results = new LinkedList<>();

        for (int v = 0; v < graph.V; v++) {
            if (!visited[v]) {
                helper(v);
            }
        }
        return results.stream().distinct().collect(Collectors.toList());
    }

    void helper(int u) {

        visited[u] = true;
        disc[u] = time;
        low[u] = time;
        ++time;

        int children = 0;
        for (int v : graph.adj[u]) {
            if (!visited[v]) {
                parent[v] = u;
                helper(v);
                children ++;

                // The top node can reach through the subtree headed by U.
                low[u] = Math.min(low[u], low[v]);

                //condition for not root node
                if ( parent[u]!=-1 && low[v]>=disc[u]){
                    /*
                    for the branch headed by node v, the top node the branch can reach is u.

                    If U is removed,  the branch can't be reach anymore.
                    Can't it be reached by cross edge or forward edge? undirected graph doesn't
                    have cross edge or forward edge.
                     */
                    //

                    results.add(u);
                }
                //condition for root node.
                if (parent[u]==-1 && children>=2) {
                    results.add(u);
                }
            } else if (parent[u]!=v ) {
                /*
                 u->v is a back edge.
                 1. Judge parent to make sure we don't use the edge twice.

                 node u can reach disc[v].
                 We don't use low[v]: low[v] represents the lowest node that can be reached
                  by all its branches.

                  It also contains the lowest node from other branches which u is not in.
                 */
                low[u] = Math.min(low[u], disc[v]);
            }
        }
    }

    public static class TestCase {
        @Test
        public void testArticulation1() {
            // Create ArticulationPoints given in above diagrams
            System.out.println("Articulation points in first ArticulationPoint ");
            ArticulationPoint articulationPoint = new ArticulationPoint();
            ConnectivityGraph g1 = new ConnectivityGraph(5);
            g1.addEdge(1, 0);
            g1.addEdge(0, 2);
            g1.addEdge(2, 1);
            g1.addEdge(0, 3);
            g1.addEdge(3, 4);
            List<Integer> points = new ArticulationPoint().find(g1);
            String result = points.stream().sorted().map(i -> i.toString()).collect(Collectors.joining(","));
            assertThat(result, equalTo("0,3"));
        }

        @Test
        public void testArticulation2() {
            System.out.println("Articulation points in Second ArticulationPoint");
            ConnectivityGraph g2 = new ConnectivityGraph(4);
            g2.addEdge(0, 1);
            g2.addEdge(1, 2);
            g2.addEdge(2, 3);
            List<Integer> points = new ArticulationPoint().find(g2);
            String result = points.stream().sorted().map(i -> i.toString()).collect(Collectors.joining(","));
            assertThat(result, equalTo("1,2"));
        }


        @Test
        public void testArticulation4() {
            System.out.println("Articulation points in Second ArticulationPoint");
            ConnectivityGraph g2 = new ConnectivityGraph(3);
            g2.addEdge(0, 1);
            g2.addEdge(0, 2);
            List<Integer> points = new ArticulationPoint().find(g2);
            String result = points.stream().sorted().map(i -> i.toString()).collect(Collectors.joining(","));
            assertThat(result, equalTo("0"));
        }
        @Test
        public void testArticulation3() {

            System.out.println("Articulation points in Third ArticulationPoint ");
            ConnectivityGraph g3 = new ConnectivityGraph(7);
            g3.addEdge(0, 1);
            g3.addEdge(1, 2);
            g3.addEdge(2, 0);
            g3.addEdge(1, 3);
            g3.addEdge(1, 4);
            g3.addEdge(1, 6);
            g3.addEdge(3, 5);
            g3.addEdge(4, 5);
            List<Integer>  points = new ArticulationPoint().find(g3);
            String result = points.stream().sorted().map(i -> i.toString()).collect(Collectors.joining(","));
            assertThat(result, equalTo("1"));
        }
    }

}
